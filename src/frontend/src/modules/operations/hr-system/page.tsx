...
/**
 * HR & Jobs System — main page (18 modules).
...
import { useHrSystem } from './useHrSystem';
import {
  HR_GROUP_ORDER,
  HR_SECTIONS,
  hrSectionFilledRatio,
  type SectionDef,
    ...
    <section className="rounded-xl border border-slate-700 bg-gradient-to-br from-slate-800/70 to-slate-900/70 p-5">
      <div className="flex items-center justify-between gap-4 mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-semibold text-white">HR & Hiring System Health</h2>
        </div>
...
function MasterDocSection({
  ...
  const generateMarkdown = (): string => {
    const lines: string[] = [];
    lines.push(`# HR & Jobs Master Document`);
    if (project?.name) lines.push(`**Programme:** ${project.name}`);
    if (project?.client) lines.push(`**Client / company:** ${project.client}`);
    lines.push(
      `*Version ${(project?.masterDocVersion ?? 0) + 1} · generated ${new Date().toISOString()}*`
    );
    lines.push('');
    for (const groupName of HR_GROUP_ORDER) {
      const inGroup = HR_SECTIONS.filter((s) => s.group === groupName);
  ...
  const onExportMd = () => {
    onBumpVersion();
    downloadFile(
      `${brand || 'hr-system'}-v${(project?.masterDocVersion ?? 0) + 1}.md`,
      generateMarkdown(),
      'text/markdown'
    );
  };
  ...
  const onExportJson = () => {
    downloadFile(
      `${brand || 'hr-system'}-v${(project?.masterDocVersion ?? 0) + 1}.json`,
    ...
    <div className="rounded-xl border border-slate-700 bg-slate-800/50 p-5">
      <div className="flex items-center justify-between mb-3 gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-white">Master HR Playbook</h3>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={onExportMd}
      ...
      <p className="text-xs text-slate-400">
        Captures every HR section into one shareable playbook.{' '}
...
export default function HrSystemPage() {
  ...
  const [activeId, setActiveId] = useState<string>(HR_SECTIONS[0].id);
  ...
  useEffect(() => {
    if (!mounted) return;
    const el = document.getElementById(`hr-section-${activeId}`);
  ...
  const stats = useMemo(() => {
    ...
    for (const sec of HR_SECTIONS) {
      ...
      totalRatio += hrSectionFilledRatio(sec, st?.data);
    }
    const overallPct = Math.round((totalRatio / HR_SECTIONS.length) * 100);
  ...
  if (!mounted) {
    return (
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl font-bold text-white">HR & Jobs — Hiring System</h1>
        <p className="mt-1 text-slate-400">
          An 18-module workflow from job creation to lifecycle management.
        </p>
  ...
  if (!activeCompanyId || !project) {
    return (
      <div className="max-w-3xl mx-auto p-8 text-center">
        <h1 className="text-2xl font-bold text-white mb-2">HR & Jobs — Hiring System</h1>
        <p className="text-slate-400">
          Create or pick a company in the sidebar to start your HR programme.
        </p>
      </div>
    );
  }

  const groupedNav = HR_GROUP_ORDER.map((group) => ({
    group,
    sections: HR_SECTIONS.filter((s) => s.group === group),
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="mb-5 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-white">HR & Jobs — Hiring System</h1>
          <p className="mt-1 text-slate-400">
            18 modules covering the full hiring + employee lifecycle from req to exit.
          </p>
        </div>
        <div className="flex flex-col md:flex-row gap-2 md:items-end">
          <div className="md:w-56">
            <Input
              ...
              placeholder="e.g. 2026 hiring plan"
      ...
      <div className="mt-6 grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-6">
              ...
              <div key={g.group} className="mb-1">
                ...
                <div className="flex flex-col gap-0.5">
                  {visible.map((s) => {
                    ...
                    const ratio = hrSectionFilledRatio(s, state?.data);
                    const Icon = s.icon;
        ...
        <div className="space-y-6">
          {HR_SECTIONS.map((s) => (
            <div key={s.id} id={`hr-section-${s.id}`}>
              <SectionForm
                section={s}
          ...
          <div className="text-xs text-slate-500 text-center pt-2 pb-8">
            HR & Jobs System · {HR_SECTIONS.length} modules · {stats.completed} completed
