...
/**
 * Website Project Tracker — main page (28 modules).
 *
 *   · Sticky grouped nav (9 groups, 28 sections)
 *   · Health Dashboard at the top — overall completion + per-status counts
 *   · Section forms rendered config-driven from sections.config.ts
 *   · Master Document export (Markdown) once enough sections are completed
 *   · Approval log
 */
...
import { useWebsiteProject } from './useWebsiteProject';
    ...
    <section className="rounded-xl border border-slate-700 bg-gradient-to-br from-slate-800/70 to-slate-900/70 p-5">
      <div className="flex items-center justify-between gap-4 mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-semibold text-white">Project Health Dashboard</h2>
        </div>
...
function MasterDocSection({
  ...
  const generateMarkdown = (): string => {
    const lines: string[] = [];
    lines.push(`# Website Project Tracker`);
    if (project?.name) lines.push(`**Project:** ${project.name}`);
    if (project?.client) lines.push(`**Client:** ${project.client}`);
    lines.push(`*Version ${(project?.masterDocVersion ?? 0) + 1} · generated ${new Date().toISOString()}*`);
    lines.push('');
    for (const groupName of GROUP_ORDER) {
      const inGroup = SECTIONS.filter((s) => s.group === groupName);
  ...
  const onExportMd = () => {
    onBumpVersion();
    downloadFile(`${brand || 'website-project'}-v${(project?.masterDocVersion ?? 0) + 1}.md`, generateMarkdown(), 'text/markdown');
  };

  const onExportJson = () => {
    downloadFile(`${brand || 'website-project'}-v${(project?.masterDocVersion ?? 0) + 1}.json`, JSON.stringify(project, null, 2), 'application/json');
  };
    ...
    <div className="rounded-xl border border-slate-700 bg-slate-800/50 p-5">
      <div className="flex items-center justify-between mb-3 gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-white">Master Project Document</h3>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={onExportMd}
      ...
      <p className="text-xs text-slate-400">
        Captures every section, status, and field value into a single shareable doc.{' '}
...
export default function WebsiteProjectPage() {
  const { project, activeCompanyId, ensureProject, updateRoot, updateField, setStatus, toggleApproval } =
    useWebsiteProject();
  ...
  useEffect(() => {
    if (!mounted) return;
    const el = document.getElementById(`web-section-${activeId}`);
  ...
  const stats = useMemo(() => {
    ...
    for (const sec of SECTIONS) {
      ...
      totalRatio += sectionFilledRatio(sec, st?.data);
    }
    const overallPct = Math.round((totalRatio / SECTIONS.length) * 100);
  ...
  if (!mounted) {
    return (
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl font-bold text-white">Website Content & Project Tracker</h1>
        <p className="mt-1 text-slate-400">
          A 28-module workflow from onboarding to ongoing maintenance.
        </p>
  ...
  if (!activeCompanyId || !project) {
    return (
      <div className="max-w-3xl mx-auto p-8 text-center">
        <h1 className="text-2xl font-bold text-white mb-2">Website Project Tracker</h1>
        <p className="text-slate-400">
          Create or pick a company in the sidebar to start a website project.
        </p>
      </div>
    );
  }
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="mb-5 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-white">Website Content & Project Tracker</h1>
          <p className="mt-1 text-slate-400">
            28 modules covering the full website lifecycle — from kick-off to maintenance.
          </p>
        </div>
      ...
      <div className="mt-6 grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-6">
              ...
              <div key={g.group} className="mb-1">
                ...
                <div className="flex flex-col gap-0.5">
                  {visible.map((s) => {
                    ...
                    const ratio = sectionFilledRatio(s, state?.data);
                    const Icon = s.icon;
        ...
        {/* Sections + master doc + approval log */}
        <div className="space-y-6">
          {SECTIONS.map((s) => (
            <div key={s.id} id={`web-section-${s.id}`}>
              <SectionForm
                section={s}
          ...
          <MasterDocSection
            ...
            onBumpVersion={() => {
              ...
              updateRoot({ masterDocVersion: v, masterDocLockedAt: new Date().toISOString() });
            }}
          />
          ...
          <div className="text-xs text-slate-500 text-center pt-2 pb-8">
            Website Project Tracker · {SECTIONS.length} modules · {stats.completed} completed
