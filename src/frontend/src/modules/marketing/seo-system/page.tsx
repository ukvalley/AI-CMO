...
/**
 * SEO System — main page (15 modules).
 * Reuses SectionForm from the website-project module so rendering is shared.
 */

import React, { useEffect, useMemo, useState } from 'react';
import { Activity, CheckCircle2, Download, FileText, Lock } from 'lucide-react';
import { cn } from '@/utils/cn';
import { Input } from '@/components/ui';
import { SectionForm } from '@/modules/website/website-project/SectionForm';
import { useSeoSystem } from './useSeoSystem';
import {
  SEO_GROUP_ORDER,
  SEO_SECTIONS,
  seoSectionFilledRatio,
  type SectionDef,
    ...
    <section className="rounded-xl border border-white/10 bg-gradient-to-br from-[#151920] to-[#0d1117] p-5">
      <div className="flex items-center justify-between gap-4 mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-semibold text-white">SEO System Health</h2>
        </div>
...
function MasterDocSection({
  ...
  const generateMarkdown = (): string => {
    const lines: string[] = [];
    lines.push(`# SEO Master Document`);
    ...
    lines.push('');
    for (const groupName of SEO_GROUP_ORDER) {
      const inGroup = SEO_SECTIONS.filter((s) => s.group === groupName);
  ...
  const onExportMd = () => {
    onBumpVersion();
    downloadFile(
      `${brand || 'seo-system'}-v${(project?.masterDocVersion ?? 0) + 1}.md`,
      generateMarkdown(),
      'text/markdown'
    );
  };
  ...
  const onExportJson = () => {
    downloadFile(
      `${brand || 'seo-system'}-v${(project?.masterDocVersion ?? 0) + 1}.json`,
    ...
    <div className="rounded-xl border border-white/10 bg-[#151920] p-5">
      <div className="flex items-center justify-between mb-3 gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-white">Master SEO Strategy</h3>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={onExportMd}
      ...
      <p className="text-xs text-[#878e9a]">
        Captures every SEO module into a single shareable strategy doc.{' '}
...
export default function SeoSystemPage() {
  ...
  const [activeId, setActiveId] = useState<string>(SEO_SECTIONS[0].id);
  ...
  useEffect(() => {
    if (!mounted) return;
    const el = document.getElementById(`seo-section-${activeId}`);
  ...
  const stats = useMemo(() => {
    ...
    for (const sec of SEO_SECTIONS) {
      ...
      totalRatio += seoSectionFilledRatio(sec, st?.data);
    }
    const overallPct = Math.round((totalRatio / SEO_SECTIONS.length) * 100);
  ...
  if (!mounted) {
    return (
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl font-bold text-white">SEO — Strategy & Execution System</h1>
        <p className="mt-1 text-[#878e9a]">
          A 15-module SEO system from onboarding to ongoing maintenance.
        </p>
  ...
  if (!activeCompanyId || !project) {
    return (
      <div className="max-w-3xl mx-auto p-8 text-center">
        <h1 className="text-2xl font-bold text-white mb-2">SEO — Strategy & Execution System</h1>
        <p className="text-[#878e9a]">
          Create or pick a company in the sidebar to start an SEO programme.
        </p>
      </div>
    );
  }

  const groupedNav = SEO_GROUP_ORDER.map((group) => ({
    group,
    sections: SEO_SECTIONS.filter((s) => s.group === group),
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="mb-5 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-white">SEO — Strategy & Execution System</h1>
          <p className="mt-1 text-[#878e9a]">
            15 modules covering the full SEO lifecycle — onboarding, audit, keywords, on-page,
            technical, off-page, local, SaaS, GEO/AEO, tracking, reporting, maintenance.
          </p>
        </div>
        <div className="flex flex-col md:flex-row gap-2 md:items-end">
          <div className="md:w-56">
            <Input
              ...
              placeholder="e.g. Q3 SEO programme"
      ...
      <div className="mt-6 grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-6">
              ...
              <div key={g.group} className="mb-1">
                ...
                <div className="flex flex-col gap-0.5">
                  {visible.map((s) => {
                    ...
                    const ratio = seoSectionFilledRatio(s, state?.data);
                    const Icon = s.icon;
        ...
        <div className="space-y-6">
          {SEO_SECTIONS.map((s) => (
            <div key={s.id} id={`seo-section-${s.id}`}>
              <SectionForm
                section={s}
          ...
          <div className="text-xs text-[#686f7e] text-center pt-2 pb-8">
            SEO System · {SEO_SECTIONS.length} modules · {stats.completed} completed
