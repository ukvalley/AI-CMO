...
/**
 * GEO / AI Search System — main page (13 modules).
 * Reuses SectionForm from the website-project module.
 *
 * NOTE: this page is mounted as a sub-tab inside the /seo route — there's no
 * standalone /geo route, the parent page handles its own DashboardLayout and
 * the SEO ↔ GEO tab toggle.
 */
...
import { useGeoSystem } from './useGeoSystem';
import {
  GEO_GROUP_ORDER,
  GEO_SECTIONS,
  geoSectionFilledRatio,
  type SectionDef,
    ...
    <section className="rounded-xl border border-slate-700 bg-gradient-to-br from-slate-800/70 to-slate-900/70 p-5">
      <div className="flex items-center justify-between gap-4 mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-semibold text-white">GEO / AI Search Health</h2>
        </div>
...
function MasterDocSection({
  ...
  const generateMarkdown = (): string => {
    const lines: string[] = [];
    lines.push(`# GEO / AI Search Master Document`);
    ...
    lines.push('');
    for (const groupName of GEO_GROUP_ORDER) {
      const inGroup = GEO_SECTIONS.filter((s) => s.group === groupName);
  ...
  const onExportMd = () => {
    onBumpVersion();
    downloadFile(
      `${brand || 'geo-system'}-v${(project?.masterDocVersion ?? 0) + 1}.md`,
      generateMarkdown(),
      'text/markdown'
    );
  };
  ...
  const onExportJson = () => {
    downloadFile(
      `${brand || 'geo-system'}-v${(project?.masterDocVersion ?? 0) + 1}.json`,
    ...
    <div className="rounded-xl border border-slate-700 bg-slate-800/50 p-5">
      <div className="flex items-center justify-between mb-3 gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-white">Master GEO Playbook</h3>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={onExportMd}
      ...
      <p className="text-xs text-slate-400">
        Captures every GEO section into one shareable playbook.{' '}
...
export default function GeoSystemPage() {
  ...
  const [activeId, setActiveId] = useState<string>(GEO_SECTIONS[0].id);
  ...
  useEffect(() => {
    if (!mounted) return;
    const el = document.getElementById(`geo-section-${activeId}`);
  ...
  const stats = useMemo(() => {
    ...
    for (const sec of GEO_SECTIONS) {
      ...
      totalRatio += geoSectionFilledRatio(sec, st?.data);
    }
    const overallPct = Math.round((totalRatio / GEO_SECTIONS.length) * 100);
  ...
  if (!activeCompanyId || !project) {
    return (
      <div className="max-w-3xl mx-auto p-8 text-center text-slate-400">
        Pick a company in the sidebar to start a GEO / AI Search programme.
      </div>
    );
  }

  const groupedNav = GEO_GROUP_ORDER.map((group) => ({
    group,
    sections: GEO_SECTIONS.filter((s) => s.group === group),
  ...
  return (
    <div>
      ...
      <div className="mb-5 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-white">GEO / AI Search System</h2>
          <p className="mt-1 text-slate-400">
            13 modules to be visible inside ChatGPT, Gemini, Perplexity & Co.
          </p>
        </div>
        <div className="flex flex-col md:flex-row gap-2 md:items-end">
          <div className="md:w-56">
            <Input
              ...
              placeholder="e.g. AI visibility plan"
      ...
      <div className="mt-6 grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-6">
              ...
              <div key={g.group} className="mb-1">
                ...
                <div className="flex flex-col gap-0.5">
                  {visible.map((s) => {
                    ...
                    const ratio = geoSectionFilledRatio(s, state?.data);
                    const Icon = s.icon;
        ...
        <div className="space-y-6">
          {GEO_SECTIONS.map((s) => (
            <div key={s.id} id={`geo-section-${s.id}`}>
              <SectionForm
                section={s}
          ...
          <div className="text-xs text-slate-500 text-center pt-2 pb-8">
            GEO / AI Search · {GEO_SECTIONS.length} modules · {stats.completed} completed
