...
/**
 * SXO (Search Experience Optimization) System — main page (20 modules).
...
import { useSxoSystem } from './useSxoSystem';
import {
  SXO_GROUP_ORDER,
  SXO_SECTIONS,
  sxoSectionFilledRatio,
  type SectionDef,
    ...
    <section className="rounded-xl border border-slate-700 bg-gradient-to-br from-slate-800/70 to-slate-900/70 p-5">
      <div className="flex items-center justify-between gap-4 mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-semibold text-white">SXO System Health</h2>
        </div>
...
function MasterDocSection({
  ...
  const generateMarkdown = (): string => {
    const lines: string[] = [];
    lines.push(`# SXO Master Document`);
    ...
    lines.push('');
    for (const groupName of SXO_GROUP_ORDER) {
      const inGroup = SXO_SECTIONS.filter((s) => s.group === groupName);
  ...
  const onExportMd = () => {
    onBumpVersion();
    downloadFile(
      `${brand || 'sxo-system'}-v${(project?.masterDocVersion ?? 0) + 1}.md`,
      generateMarkdown(),
      'text/markdown'
    );
  };
  ...
  const onExportJson = () => {
    downloadFile(
      `${brand || 'sxo-system'}-v${(project?.masterDocVersion ?? 0) + 1}.json`,
    ...
    <div className="rounded-xl border border-slate-700 bg-slate-800/50 p-5">
      <div className="flex items-center justify-between mb-3 gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-white">Master SXO Playbook</h3>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={onExportMd}
      ...
      <p className="text-xs text-slate-400">
        Captures every SXO section into one shareable playbook.{' '}
...
export default function SxoSystemPage() {
  const {
    project,
    activeCompanyId,
    ensureProject,
  ...
  const [activeId, setActiveId] = useState<string>(SXO_SECTIONS[0].id);
  ...
  useEffect(() => {
    if (!mounted) return;
    const el = document.getElementById(`sxo-section-${activeId}`);
  ...
  const stats = useMemo(() => {
    ...
    for (const sec of SXO_SECTIONS) {
      ...
      totalRatio += sxoSectionFilledRatio(sec, st?.data);
    }
    const overallPct = Math.round((totalRatio / SXO_SECTIONS.length) * 100);
  ...
  if (!activeCompanyId || !project) {
    return (
      <div className="max-w-3xl mx-auto p-8 text-center text-slate-400">
        Pick a company in the sidebar to start an SXO programme.
      </div>
    );
  }

  const groupedNav = SXO_GROUP_ORDER.map((group) => ({
    group,
    sections: SXO_SECTIONS.filter((s) => s.group === group),
  ...
  return (
    <div>
      <div className="mb-5 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-white">SXO — Search Experience Optimization</h2>
          <p className="mt-1 text-slate-400">
            20 modules unifying SEO, UX, and CRO into one revenue-aligned programme.
          </p>
        </div>
        <div className="flex flex-col md:flex-row gap-2 md:items-end">
          <div className="md:w-56">
            <Input
              ...
              placeholder="e.g. SXO 2026 programme"
      ...
      <div className="mt-6 grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-6">
              ...
              <div key={g.group} className="mb-1">
                ...
                <div className="flex flex-col gap-0.5">
                  {visible.map((s) => {
                    ...
                    const ratio = sxoSectionFilledRatio(s, state?.data);
                    const Icon = s.icon;
        ...
        <div className="space-y-6">
          {SXO_SECTIONS.map((s) => (
            <div key={s.id} id={`sxo-section-${s.id}`}>
              <SectionForm
                section={s}
          ...
          <div className="text-xs text-slate-500 text-center pt-2 pb-8">
            SXO System · {SXO_SECTIONS.length} modules · {stats.completed} completed
