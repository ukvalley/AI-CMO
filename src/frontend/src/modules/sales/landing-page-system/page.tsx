...
/**
 * Landing Page System — main page (19 modules).
...
import { useLandingPageSystem } from './useLandingPageSystem';
import {
  LP_GROUP_ORDER,
  LP_SECTIONS,
  lpSectionFilledRatio,
  type SectionDef,
...
function MasterDocSection({
  ...
  const generateMarkdown = (): string => {
    const lines: string[] = [];
    lines.push(`# Landing Page Master Document`);
    ...
    lines.push('');
    for (const groupName of LP_GROUP_ORDER) {
      const inGroup = LP_SECTIONS.filter((s) => s.group === groupName);
  ...
  const onExportMd = () => {
    onBumpVersion();
    downloadFile(
      `${brand || 'landing-page'}-v${(project?.masterDocVersion ?? 0) + 1}.md`,
      generateMarkdown(),
      'text/markdown'
    );
  };
  ...
  const onExportJson = () => {
    downloadFile(
      `${brand || 'landing-page'}-v${(project?.masterDocVersion ?? 0) + 1}.json`,
...
export default function LandingPageSystemPage() {
  const {
    project,
    activeCompanyId,
    ensureProject,
    ...
    setStatus,
    toggleApproval,
  } = useLandingPageSystem();
  const [activeId, setActiveId] = useState<string>(LP_SECTIONS[0].id);
  ...
  useEffect(() => {
    if (!mounted) return;
    const el = document.getElementById(`lp-section-${activeId}`);
  ...
  const stats = useMemo(() => {
    ...
    for (const sec of LP_SECTIONS) {
      ...
      totalRatio += lpSectionFilledRatio(sec, st?.data);
    }
    const overallPct = Math.round((totalRatio / LP_SECTIONS.length) * 100);
  ...
  const groupedNav = LP_GROUP_ORDER.map((group) => ({
    group,
    sections: LP_SECTIONS.filter((s) => s.group === group),
    ...
    <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row gap-2 md:items-end">
          <div className="md:w-56">
            <Input
              ...
              placeholder="e.g. Spring webinar LP"
              label="Project name"
            />
          </div>
          <div className="md:w-56">
      ...
      <div className="mt-6 grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-6">
              ...
              <div key={g.group} className="mb-1">
                ...
                <div className="flex flex-col gap-0.5">
                  {visible.map((s) => {
                    ...
                    const ratio = lpSectionFilledRatio(s, state?.data);
                    const Icon = s.icon;
        ...
        <div className="space-y-6">
          {LP_SECTIONS.map((s) => (
            <div key={s.id} id={`lp-section-${s.id}`}>
              <SectionForm
                section={s}
