...
/**
 * Sales Script Engine — main page (7 sections).
 * Reuses SectionForm from the website-project module.
 */
...
import { useSalesScriptEngine } from './useSalesScriptEngine';
import {
  SCRIPT_GROUP_ORDER,
  SCRIPT_SECTIONS,
  scriptSectionFilledRatio,
  type SectionDef,
...
function HealthDashboard({
  ...
  const cards = [
    { label: 'Total sections', value: total, tone: 'slate' as const },
    ...
    <section className="rounded-xl border border-slate-700 bg-gradient-to-br from-slate-800/70 to-slate-900/70 p-5">
      <div className="flex items-center justify-between gap-4 mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-semibold text-white">Script Engine Health</h2>
        </div>
...
function MasterDocSection({
  ...
  const generateMarkdown = (): string => {
    const lines: string[] = [];
    lines.push(`# Sales Script Playbook`);
    if (project?.name) lines.push(`**Programme:** ${project.name}`);
    if (project?.client) lines.push(`**Client / division:** ${project.client}`);
    lines.push(
      `*Version ${(project?.masterDocVersion ?? 0) + 1} · generated ${new Date().toISOString()}*`
    );
    lines.push('');
    for (const groupName of SCRIPT_GROUP_ORDER) {
      const inGroup = SCRIPT_SECTIONS.filter((s) => s.group === groupName);
  ...
  const onExportMd = () => {
    onBumpVersion();
    downloadFile(
      `${brand || 'sales-scripts'}-v${(project?.masterDocVersion ?? 0) + 1}.md`,
      generateMarkdown(),
      'text/markdown'
    );
  };
  ...
  const onExportJson = () => {
    downloadFile(
      `${brand || 'sales-scripts'}-v${(project?.masterDocVersion ?? 0) + 1}.json`,
    ...
    <div className="rounded-xl border border-slate-700 bg-slate-800/50 p-5">
      <div className="flex items-center justify-between mb-3 gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-white">Master Sales Script Playbook</h3>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={onExportMd}
      ...
      <p className="text-xs text-slate-400">
        Captures every script in this rep&apos;s playbook into one shareable doc.{' '}
...
export default function SalesScriptEnginePage() {
  const {
    project,
    activeCompanyId,
    ensureProject,
    ...
    setStatus,
    toggleApproval,
  } = useSalesScriptEngine();
  const [activeId, setActiveId] = useState<string>(SCRIPT_SECTIONS[0].id);
  ...
  useEffect(() => {
    if (!mounted) return;
    const el = document.getElementById(`script-section-${activeId}`);
  ...
  const stats = useMemo(() => {
    ...
    for (const sec of SCRIPT_SECTIONS) {
      ...
      totalRatio += scriptSectionFilledRatio(sec, st?.data);
    }
    const overallPct = Math.round((totalRatio / SCRIPT_SECTIONS.length) * 100);
  ...
  if (!mounted) {
    return (
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl font-bold text-white">Sales Script Engine</h1>
        <p className="mt-1 text-slate-400">
          Personalised scripts per rep — outbound, discovery, demo, objections, closing, retention.
        </p>
  ...
  if (!activeCompanyId || !project) {
    return (
      <div className="max-w-3xl mx-auto p-8 text-center">
        <h1 className="text-2xl font-bold text-white mb-2">Sales Script Engine</h1>
        <p className="text-slate-400">
          Pick a company in the sidebar to start a script playbook.
        </p>
      </div>
    );
  }

  const groupedNav = SCRIPT_GROUP_ORDER.map((group) => ({
    group,
    sections: SCRIPT_SECTIONS.filter((s) => s.group === group),
    ...
    <div className="max-w-7xl mx-auto">
      <div className="mb-5 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-white">Sales Script Engine</h1>
          <p className="mt-1 text-slate-400">
            Personalised scripts per rep — outbound, discovery, demo, objections, closing,
            relationship.
          </p>
        </div>
        <div className="flex flex-col md:flex-row gap-2 md:items-end">
          <div className="md:w-56">
            <Input
              ...
              placeholder="e.g. Q3 SDR playbook"
      ...
      <div className="mt-6 grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-6">
              ...
              <div key={g.group} className="mb-1">
                ...
                <div className="flex flex-col gap-0.5">
                  {visible.map((s) => {
                    ...
                    const ratio = scriptSectionFilledRatio(s, state?.data);
                    const Icon = s.icon;
        ...
        <div className="space-y-6">
          {SCRIPT_SECTIONS.map((s) => (
            <div key={s.id} id={`script-section-${s.id}`}>
              <SectionForm
                section={s}
          ...
          <div className="text-xs text-slate-500 text-center pt-2 pb-8">
            Sales Script Engine · {SCRIPT_SECTIONS.length} sections · {stats.completed} completed
