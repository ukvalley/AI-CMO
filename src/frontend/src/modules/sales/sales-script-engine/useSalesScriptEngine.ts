/**
 * useSalesScriptEngine — singleton hook for the 7-section script engine.
...
const DEFAULT_SCRIPT_ENGINE = (companyId: string): SalesScriptEngine => {
  const now = new Date().toISOString();
  return {
    id: `sales-script-engine-${Date.now()}`,
    companyId,
    createdAt: now,
    updatedAt: now,
    name: '',
...
export function useSalesScriptEngine() {
  ...
  const fromStore = useDataStore((s) =>
    activeCompanyId ? s.data[activeCompanyId]?.salesScriptEngine ?? null : null
  );

  const fallback = useMemo<SalesScriptEngine | null>(
    () => (activeCompanyId ? DEFAULT_SCRIPT_ENGINE(activeCompanyId) : null),
    [activeCompanyId]
  );

  const project: SalesScriptEngine | null = fromStore ?? fallback;

  const ensureProject = (): SalesScriptEngine => {
    if (!activeCompanyId) {
    ...
    const existing = getItems('salesScriptEngine') as SalesScriptEngine | null;
    if (existing) return existing;
    const seeded = DEFAULT_SCRIPT_ENGINE(activeCompanyId);
    setItems('salesScriptEngine', seeded);
    return seeded;
  };

  const updateRoot = (patch: Partial<SalesScriptEngine>) => {
    if (!activeCompanyId) return;
    if (dataActiveId !== activeCompanyId) {
      setDataActiveCompany(activeCompanyId);
    }
    const current =
      (getItems('salesScriptEngine') as SalesScriptEngine | null) ?? ensureProject();
    setItems('salesScriptEngine', {
      ...current,
      ...patch,
